...
import com.myapp.convfl.constants.SystemConstants;
...
public final class CloudStorageUtility {
    ...
    public static byte[] download(CloudEvent event) throws Exception {
        ...
        Storage storage = StorageOptions.newBuilder()
                .setProjectId(System.getenv(SystemConstants.SYS_ENV_KEY_PROJECT_ID))
                .build().getService();
    ...
    public static boolean upload(CloudEvent event, String uploadContent, FeedType feedType) throws Exception {
        String cloudEventData = new String(event.getData().toBytes(), StandardCharsets.UTF_8);
        StorageObjectData.Builder builder = StorageObjectData.newBuilder();
        JsonFormat.parser().merge(cloudEventData, builder);
        StorageObjectData data = builder.build();
        ...
        try {
        // ファイルを指定のバケットにアップロード
        BlobId blobId = BlobId.of(feedType.getExifDef().getOutBucket(), uploadFileName);
